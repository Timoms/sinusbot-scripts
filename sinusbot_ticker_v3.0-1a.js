registerPlugin({
	name: '(News-)Ticker',
	version: '3.0-1a',
	description: 'running text on channel base with news support. User ".tickeron" or ".tickeroff" to toggle the ticker."',
	author: 'Created by Timo Heckel <info@ompan.net> | Reworked by Lukas Westholt <lukaswestholt@yahoo.de>',
	vars: [{
			name: 'type',
		...
		},
		{
            ...
            type: 'string',
			placeholder: 'for example: http://api.ompan.de/txt.txt',
			indent: 2,
			conditions: [
		...
		setInterval(function() {
            ...
            function Process(error, response) {
                ...
                if (response.statusCode != 200) {
                    engine.log("Sorry, Your API/Website is currently not available. Please try again later or contact info@ompan.net for help.");
                    return;
                } else {
                    msg_g = response.data;
